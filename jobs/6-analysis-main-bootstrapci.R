...
# FILE: ANALYSES, BOOTSTRAPPED ESTIMATES, MAIN DECOMPOSITION
# AUTHOR: NINO CRICCO
#------------------------------------------------------------------------------
...
for(i in 1:10000){
  ...
  saveRDS(main_decomp, file = paste0(
    "bootstrap_maindecomp/bootstrap_decomp_", date_run_id, "_", i, ".RDS"))
...
ta3 <- read_csv("tables/tablea3.csv") %>% 
              transmute(Model, Variable, 
                        ...
                        Total_Interaction, Men_Interaction, Women_Interaction) %>%
              mutate_if(is.numeric, as.character) %>%
  bind_rows(read_csv("tables/bootstrap_maindecomp.csv") %>%
...
kable(ta3 %>% filter(Variable %in% c("num.kids.cont", "Total")), format = "latex")

kable(ta3 %>% select(Model, Variable, ends_with("Gap")) %>% 
